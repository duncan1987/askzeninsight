...
export default function NewBlogPage() {
  ...
  useEffect(() => {
    if (fromCourseId && adminKey) {
      setLoadingCourse(true)
      fetch(`/api/admin/blog/posts?fromCourseId=${fromCourseId}`, {
        ...
        .then((data) => {
          if (data.course) {
            ...
            setSlug(slugify(data.course.title))
    ...
    try {
      ...
      const res = await fetch("/api/admin/blog/posts", {
        ...
        body: JSON.stringify({
          ...
          source_course_id: fromCourseId || undefined,
        }),
      })
      if (res.ok) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold">新建博客文章</h1>
          {fromCourseId && (
            <span className="text-sm text-muted-foreground">（从课程导入）</span>
          )}
        </div>
