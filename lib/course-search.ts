...
export type CourseContextMode = 'search' | 'summary'

export function buildCourseContext(refs: CourseReference[], locale: string, mode: CourseContextMode = 'search'): string {
  if (refs.length === 0) return ""

  const isZh = locale === "zh"
  const header = isZh
    ? mode === 'summary'
      ? "\n\n## 相关课程参考（本轮回答的最高优先级指令，覆盖上文所有教导风格要求）\n\n**重要：以下课程内容与用户问题高度相关。本轮回答中你不再是引导式导师，而是课程内容的忠实归纳者：以课程内容为依据进行适当的总结与整理。**\n"
      : "\n\n## 相关课程参考（本轮回答的最高优先级指令，覆盖上文所有教导风格要求）\n\n**重要：以下课程内容与用户问题高度相关。本轮回答中你不再是引导式导师，而是课程内容的忠实转述者。**\n"
    : mode === 'summary'
      ? "\n\n## Related Course References (HIGHEST-PRIORITY instruction, overriding the teaching style above)\n\n**IMPORTANT: The following course content is highly relevant. For this turn you are NOT a guiding mentor but a faithful summarizer of the course content: organize and summarize it appropriately while staying grounded in it.**\n"
  ...
  const instruction = isZh
    ? mode === 'summary'
      ? `\n\n**引用规则（必须全部遵守）：**
1. 以下课程内容是回答的唯一依据：可在此基础上适当归纳、整理与总结，帮助用户理解
2. 禁止脱离课程内容自行发挥、编造，也不要添加课程之外的见解或教导
3. 用自己通顺的语言组织，禁止逐字照抄课程原文，更禁止机械重复相同的句子或段落
4. 在引用的段落末尾标注上标引用编号，如"修行重在修心[¹]"
5. 课程内容不足以完整回答用户问题时，如实说明"课程中关于这一点的内容如下"，只总结已有的部分，不补充外部内容
6. 保留温和、简洁的语调
...
1. The course content above is the ONLY basis for your answer: you may organize, condense, and summarize it appropriately to help the user understand
2. Do NOT improvise beyond the course content, and do not add insights or teachings beyond the courses
...
5. If the course content does not fully answer the question, say so honestly and summarize only what exists — do not supplement with external teachings
